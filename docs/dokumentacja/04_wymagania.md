...
Frontend React udostępnia widoki listy Golden Record, szczegółów osoby i podmiotu, lineage, historii zmian, wyników walidacji oraz matchingu oparte na endpointach warstwy `serving`. Pozwala filtrować listę Golden Record po typie encji i frazie wyszukiwania, przeglądać kandydatów Levenshteina i Jaro-Winklera oraz porównywać szczegóły wybranej pary rekordów. Nadal nie zawiera interfejsu ręcznej obsługi przypadków `REVIEW` ani operacji zapisu, dlatego wymaganie pozostaje zrealizowane częściowo.
...
| F-23 | Interfejs użytkownika do danych | Częściowo zrealizowane | `frontend/src/App.jsx`, `frontend/src/features/golden-records/GoldenRecordsView.jsx`, `frontend/src/features/golden-records/GoldenRecordDetailModal.jsx`, `frontend/src/features/validation/ValidationView.jsx`, `frontend/src/features/matching/MatchingView.jsx`, `frontend/src/features/matching/MatchingComparisonPanel.jsx` | Brak operacji zapisu, obsługi `REVIEW` i routingu ekranów |
...
| N-07 | Testy funkcjonalne | Zrealizowane w kodzie testów | Katalog `tests/` obejmuje główne reguły biznesowe | Repozytorium zawiera zestaw testów automatycznych dla backendu i warstwy serving, ale dokumentacja nie opiera odbioru na ręcznie utrzymywanej liczbie testów |
